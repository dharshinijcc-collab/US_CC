...
export const RENDER_API_URL = "http://localhost:4000/api";
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    const isLocalhost = config.baseURL?.includes('localhost') || config.url?.includes('localhost');
    const isTimeoutOrNetworkError =
      error.code === 'ECONNABORTED' ||
      (error.code === 'ERR_NETWORK' && !isLocalhost) ||
...
api.interceptors.request.use(
  (config) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      let token = localStorage.getItem('admin-token') || localStorage.getItem('Dtoken');
