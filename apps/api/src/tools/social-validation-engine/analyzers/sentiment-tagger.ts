...
export async function tagSentiment(sources: SourceRow[]): Promise<any> {
      ...
      const response = await withTimeout(
        ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: _SENTIMENT_SCHEMA as any
          }
        }),
        SETTINGS.sentimentTimeoutMs,
        'sentiment_tagger'
      );
  ...
  for (const tag of tags) {
    ...
    if (idx === undefined || !idMap[idx]) continue;
    ...
    if (tag.buying_intent)  buyingIntentCount++;
    if (tag.active_search)  activeSearchCount++;
  }
