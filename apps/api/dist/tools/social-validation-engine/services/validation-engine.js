...
exports.computeValidationScore = computeValidationScore;
const config_1 = require("../config");
function sigmoidNormalize(value, midpoint = 50.0, steepness = 0.05) {
    return 1 / (1 + Math.exp(-steepness * (value - midpoint)));
}
function clamp(value, lo = 0.0, hi = 1.0) {
    return Math.max(lo, Math.min(hi, value));
}
function scorePainPointFrequency(painPoints) {
...
function scoreBuyingIntent(sentiment) {
...
function scoreCompetitorWeakness(painPoints, competitors) {
...
function getVerdictFromScore(score) {
    for (const [lo, hi, label] of config_1.VERDICT_BANDS) {
...
function computeValidationScore(painPoints, sentiment, competitors, features, sources) {
    ...
    const w = config_1.SCORING_WEIGHTS;
    const rawScore = f1 * w.pain_point_frequency +
        f2 * w.buying_intent +
