...
exports.default = toolConfigRoutes;
const supabase_1 = require("../services/supabase");
const auth_1 = require("../middleware/auth");
async function toolConfigRoutes(app) {
    // GET /tool-config
    ...
    // POST /tool-config
    app.post('/tool-config', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            const { key, config } = request.body;
            if (!key || !config) {
