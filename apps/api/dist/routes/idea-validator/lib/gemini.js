...
exports.generateMockNarratives = generateMockNarratives;
...
function generateMockNarratives(idea, answers, scores, dimensions, redFlags) {
    const getDimensionLabel = (key) => {
        return scoring_1.DIMENSION_META.find(d => d.key === key)?.label || key;
    };
    ...
    Object.entries(dimensions).forEach(([key, dim]) => {
        ...
        if (key === 'build_timeline') {
            ...
            dimensionNarratives[key] = {
                ...
                evaluation_criteria: dim.factors.map((f) => f.label),
    ...
    const mockComps = startups_1.CURATED_STARTUPS.slice(0, 3).map(c => ({
        name: c.name,
...
async function generateNarrative(idea, answers, scores, dimensions, redFlags, apiKey, customPrompt, realCompetitors) {
    const activeClient = gemini_1.geminiClient;
    if (!activeClient) {
        throw new Error('Gemini client not initialized. Please configure a valid GEMINI_API_KEY.');
    }
    try {
        const ai = activeClient;
        const compsDb = realCompetitors && realCompetitors.length > 0 ? realCompetitors : startups_1.CURATED_STARTUPS;
        const compsTitle = realCompetitors && realCompetitors.length > 0
            ? "DATABASE OF REAL COMPETITORS DISCOVERED FOR THIS STARTUP (Select exactly 3-5 that best match the sector of this startup to feature in comparable_startups):"
...
- Moat: ${answers.moat}`;
        const response = await (0, gemini_1.withTimeout)(ai.models.generateContent({
            model: gemini_1.GEMINI_MODELS.standard,
            contents: [systemPrompt, prompt],
            config: {
                responseMimeType: 'application/json',
                temperature: 0.6,
            }
        }), 35000, 'Gemini narrative generation timed out');
