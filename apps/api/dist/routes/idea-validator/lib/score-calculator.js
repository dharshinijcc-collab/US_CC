...
exports.calculateAggregatedScores = calculateAggregatedScores;
function calculateAggregatedScores(dimensions, answers, redFlagCount, config) {
    ...
    const startupQualityScore = (wQuality.demand ?? 0.35) * customerDemand +
    ...
    const investorReadinessScore = (wReadiness.appeal ?? 0.40) * investorAppeal +
    ...
    // 5. Triage Band
    let triageBand = 'Promising / Needs Work';
    ...
    let valScore = 0;
    if (answers.validation_level === 'conversations')
        valScore = 2;
        ...
        valScore = 6;
    let stageScore = 0;
    if (answers.current_stage === 'ux_design')
        stageScore = 1;
    ...
    if (answers.current_stage === 'prototype')
        stageProbBonus = 5;
    else if (answers.current_stage === 'mvp')
        stageProbBonus = 10;
    let mktProbBonus = 0;
    ...
    let fundProbBonus = 0;
    if (answers.funding_status === 'bootstrapped')
        fundProbBonus = 5;
    else if (answers.funding_status === 'raising')
        fundProbBonus = 8;
    else if (answers.funding_status === 'raised')
        fundProbBonus = 10;
    ...
    const signal_confidence = Math.round(Object.values(dimensions).reduce((acc, cur) => acc + cur.confidence, 0) / Object.keys(dimensions).length);
    const evidence_confidence = evidenceScore * 10;
    const rule_confidence = 100;
    const overall_confidence = Math.min(98, Math.max(40, Math.round(0.15 * questionnaire_completeness +
