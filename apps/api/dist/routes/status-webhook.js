...
exports.default = statusWebhookRoutes;
const supabase_1 = require("../services/supabase");
const email_1 = require("../services/email");
async function statusWebhookRoutes(app) {
    // POST /status-webhook
    app.post('/status-webhook', async (request, reply) => {
        try {
            const body = request.body;
            const { record, old_record, type, table } = body;
            request.log.info(`[Status Webhook] Received webhook event. Type: ${type}, Table: ${table}`);
            ...
            if (type !== 'UPDATE') {
                return reply.send({ success: true, message: 'Ignored: Event is not an UPDATE' });
            }
            if (!record || !record.id || !record.email) {
                return reply.send({ success: true, message: 'Ignored: Missing record details or email' });
            }
            ...
            request.log.info(`[Status Webhook] Status changed detection. rawOldStatus: ${rawOldStatus}, oldStatus (normalized): ${oldStatus}, newStatus: ${newStatus}`);
            // 1. Guard: Only proceed if status actually changed
            if (rawOldStatus === newStatus) {
                request.log.info(`[Status Webhook] Status did not change (raw old: ${rawOldStatus} === new: ${newStatus}). Skipping.`);
                return reply.send({ success: true, message: 'Skipped: Status unchanged' });
            }
            // 2. Guard: Prevent duplicate emails for the same status (Idempotency)
            if (record.notification_type === newStatus) {
                request.log.info(`[Status Webhook] Notification for "${newStatus}" already sent. Skipping.`);
                return reply.send({ success: true, message: 'Skipped: Notification already sent' });
            }
        ...
        `;
            ...
            if (!isTransitionValid) {
                request.log.info(`[Status Webhook] No valid transition matched (${oldStatus} -> ${newStatus}). Skipping notification.`);
                return reply.send({ success: true, message: 'Skipped: No valid transition matched' });
            }
            // 4. Send the email via Resend
            request.log.info(`[Status Webhook] Sending email to ${record.email} (Subject: "${emailSubject}")`);
            const emailResult = await email_1.resend.emails.send({
                from: email_1.FROM_EMAIL,
                to: record.email,
                subject: emailSubject,
                html: emailHtml
            });
            request.log.info(`[Status Webhook] Resend API success. Email ID: ${emailResult.data?.id}`);
            // 5. Log details back to Supabase to update metadata columns
            request.log.info(`[Status Webhook] Logging metadata back to Supabase for record ${record.id}`);
            const { error: dbError } = await supabase_1.supabaseAdmin
                ...
                .eq('id', record.id);
            if (dbError) {
                request.log.error(dbError, '[Status Webhook] Supabase update logging error');
                return reply.status(500).send({ error: dbError.message });
            }
            request.log.info(`[Status Webhook] Logging complete for record ${record.id}`);
            return reply.send({ success: true, emailId: emailResult.data?.id });
        }
        catch (err) {
            request.log.error(err, '[Status Webhook] Webhook handler error');
