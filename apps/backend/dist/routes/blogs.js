...
async function deleteBlogCoverImage(imageUrl) {
    if (!imageUrl || !supabase_1.supabaseAdmin || !imageUrl.includes('/storage/v1/object/public/blogs/'))
        return;
    try {
        const filename = imageUrl.split('/').pop();
        if (filename) {
            const { error } = await supabase_1.supabaseAdmin.storage.from('blogs').remove([filename]);
            if (error)
                console.error('Failed to remove storage file:', error.message);
        }
    }
    catch (err) {
...
async function blogsRoutes(app) {
    // GET /blogs
    ...
    // POST /blogs
    app.post('/blogs', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
    ...
    // PUT /blogs
    app.put('/blogs', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
    ...
    // DELETE /blogs
    app.delete('/blogs', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
    ...
    // POST /blogs/upload
    app.post('/blogs/upload', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
            ...
            const uniqueName = `blog-${crypto_1.default.randomUUID()}.${ext}`;
    ...
    app.get('/blog-authors', async (request, reply) => {
        try {
            ...
            let query = supabase_1.supabaseAdmin.from('blog_authors').select('*').order('name', { ascending: true });
            if (!showAll) {
    ...
    // POST /blog-authors
    app.post('/blog-authors', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
    ...
    // PUT /blog-authors
    app.put('/blog-authors', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
    ...
    // DELETE /blog-authors
    app.delete('/blog-authors', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
