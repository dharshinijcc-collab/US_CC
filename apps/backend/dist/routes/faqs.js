...
exports.default = faqsRoutes;
const supabase_1 = require("../services/supabase");
const auth_1 = require("../middleware/auth");
async function faqsRoutes(app) {
    // GET /faqs
    app.get('/faqs', async (request, reply) => {
        try {
            ...
            let query = supabase_1.supabaseAdmin.from('faqs').select('*').order('display_order', { ascending: true });
            if (!showAll) {
    ...
    // POST /faqs
    app.post('/faqs', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
    ...
    // PUT /faqs
    app.put('/faqs', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
    ...
    // DELETE /faqs
    app.delete('/faqs', { preHandler: [auth_1.verifyAdmin] }, async (request, reply) => {
        try {
            if (!supabase_1.supabaseAdmin) {
                return reply.status(503).send({ status: 'error', message: 'Database not configured' });
            }
