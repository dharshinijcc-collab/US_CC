...
export default async function submissionsRoutes(app: FastifyInstance) {
  // GET /submissions
  app.get('/submissions', { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      if (!supabaseAdmin) {
        return reply.status(503).send({ error: 'Database not configured' });
      }

      const { formType = 'all', status = 'all', query = '', page = '1', limit = '10' } = request.query as any;
      ...
      let dbQuery = supabaseAdmin.from('submissions').select('*', { count: 'exact' });
  ...
  app.get('/submissions/details', { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      if (!supabaseAdmin) {
        return reply.status(503).send({ error: 'Database not configured' });
      }
  ...
  app.post('/submissions/update', { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      if (!supabaseAdmin) {
        return reply.status(503).send({ error: 'Database not configured' });
      }

      const { id, status, assigned_reviewer, internal_notes } = request.body as any;
  ...
  app.post('/submissions/notes', { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      if (!supabaseAdmin) {
        return reply.status(503).send({ error: 'Database not configured' });
      }

      const { submission_id, note } = request.body as any;
      const adminEmail = (request as any).user.email;
