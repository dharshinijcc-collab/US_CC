      ...
      if (request.isMultipart()) {
        ...
        for await (const part of parts) {
          if (part.type === 'file') {
            ...
            if (part.fieldname === 'firstName') firstName = (part.value as string).trim();
            if (part.fieldname === 'email') email = (part.value as string).trim();
            if (part.fieldname === 'interest') interest = (part.value as string).trim();
            if (part.fieldname === 'linkedin') linkedin = (part.value as string).trim();
          }
        }
      } else {
        const body = request.body as any;
