...
// GET /api/faqs
export async function getFaqsHandler(req: NextRequest) {
  try {
...
export async function createFaqHandler(req: NextRequest) {
  ...
  try {
    ...
    if (!body.category || !body.question || !body.answer) {
      return NextResponse.json({ status: 'error', message: 'Category, Question, and Answer are required' }, { status: 400 });
    }
...
// PUT /api/faqs
export async function updateFaqHandler(req: NextRequest) {
  if (!await authenticate(req)) return unauthorized();
  if (!supabaseAdmin) return noDb();
...
// DELETE /api/faqs
export async function deleteFaqHandler(req: NextRequest) {
  if (!await authenticate(req)) return unauthorized();
  if (!supabaseAdmin) return noDb();
  ...
  try {
    ...
    if (error) return NextResponse.json({ status: 'error', message: error.message }, { status: 500 });
    return NextResponse.json({ status: 'success', message: 'FAQ deleted successfully' });
  } catch (err: any) {
    return NextResponse.json({ status: 'error', message: err.message }, { status: 500 });
  }
}
